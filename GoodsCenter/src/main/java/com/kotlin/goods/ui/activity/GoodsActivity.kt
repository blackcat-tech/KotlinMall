...
class GoodsActivity : BaseMvpActivity<GoodsListPresenter>(), GoodsListView , BGARefreshLayout.BGARefreshLayoutDelegate{
    ...
    private fun loadData() {
        if (intent.getIntExtra(GoodsConstant.KEY_SEARCH_GOODS_TYPE, 0) != 0) {
            ...
            mPresenter.getGoodsListByKeyword(this,intent.getStringExtra(GoodsConstant.KEY_GOODS_KEYWORD), mCurrentPage)
        } else {
            mMultiStateView.startLoading()
            mPresenter.getGoodsList(this,intent.getIntExtra(GoodsConstant.KEY_CATEGORY_ID, 1), mCurrentPage)
        }
    }
    ...
    override fun injectComponent() {
        DaggerGoodsComponent.builder().goodsModule(GoodsModule()).build().inject(this)
