...
class CategoryFragment : BaseMvpFragment<CategoryPresenter>(), CategoryView {
    ...
    override fun injectComponent() {
        DaggerCategoryComponent.builder().categoryModule(CategoryModule()).build().inject(this)
    ...
    private fun loadData(parentId: Int = 0) {
        if (parentId != 0) {//只在二级分类中加载动画
            mMultiStateView.startLoading()
        }

        mPresenter.getCategory(context!!, parentId)
    ...
    override fun onGetCategoryResult(result: MutableList<Category>?) {
        ...
        if (result != null && result.size > 0) {
            if (result[0].parentId == 0) {//一级分类
                ...
                mPresenter.getCategory(context!!, result[0].id)
            } else {//二级分类
